...
void RegisterFeedbackFromJsonReflected(const char* key, const char* projectJson)
{
	auto parsed = nlohmann::json::parse(projectJson);
...
void RegisterFeedbackFromTactFileReflected(const char* key, const char* tactFileStr)
{
	...
	RegisterFeedbackFromJsonReflected(key, hapticFile.ProjectJson.c_str());
}
...
void LoadAndRegisterFeedbackReflected(const char* Key, const char* FilePath)
{
	...
	RegisterFeedbackFromJsonReflected(Key, file.ProjectJson.c_str());
}
